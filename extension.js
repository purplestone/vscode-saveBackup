...
function backupFile(document) {
    ...
    if (oConf.enable) {
        var sBakPath = buildeBakPath(sFilePath, oConf.backupDir);
        
        if ($$fs.existsSync(oConf.backupDir)) {
            try {
            ...
            vscode.window.showErrorMessage(`extension.saveBackup : ${oConf.backupDir} is not exists. To create the dir, or configure saveBackup.conf.backupDir`);
        }
    }
}
